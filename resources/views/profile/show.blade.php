<x-app-layout>
    ...
    <div class="flex flex-col min-h-screen bg-gray-50/50" x-data="{
                ...
                <div
                    ...
                    <div class="relative px-6 pb-2 flex flex-col items-center -mt-20">
                        <div class="relative group cursor-pointer z-30" @click="showPhotoModal = true">
                            <div
                                class="w-40 h-40 rounded-full border-[6px] border-white shadow-2xl overflow-hidden bg-white flex items-center justify-center relative z-10 transition transform group-hover:scale-105 duration-500">
                                ...
                                <div
                                    ...
                                    <svg xmlns="http://www.w3.org/2000/svg"
                                        class="h-10 w-10 text-white opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                        ...
                        <div class="flex flex-row justify-between items-center mb-8">
                            ...
                            <div class="flex gap-2">
                                ...
                                <div x-show="isEditing" class="flex gap-2" style="display: none;">
                                    <button type="button" @click="isEditing = false; init()"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                            ...
                            <div
                                ...
                                <div x-show="isEditing" style="display: none;">
                                    <input type="text" name="username" x-model="usernameQuery"
                                        @input="usernameQuery = $event.target.value.toLowerCase().replace(/\s/g, '')"
                                        @keyup.debounce.500ms="checkUsername()"
                                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm lowercase"
                                    ...
                                    <div class="mt-1 text-xs font-bold flex items-center">
                                        ...
                                        <span x-show="usernameStatus === 'checking'"
                                        ...
                                        <span
                                            x-show="usernameStatus === 'available' && usernameQuery !== '{{ Auth::user()->username }}'"
                                        ...
                                        <span x-show="usernameStatus === 'taken'"
                            ...
                            <div
                                class="p-4 bg-gray-50 rounded-xl border border-gray-100 hover:border-green-200 transition group">
                                <label class="text-xs font-bold text-gray-400 uppercase tracking-wider block mb-1">Nama
                                    Lengkap</label>
                                <div x-show="!isEditing"
                                    class="flex items-center text-gray-900 font-semibold text-base">
                            ...
                            <div
                                class="p-4 bg-gray-50 rounded-xl border border-gray-100 hover:border-green-200 transition group">
                                <label
                                    class="text-xs font-bold text-gray-400 uppercase tracking-wider block mb-1">Role</label>
                                <span
                            ...
                            <div class="relative z-10">
                                ...
                                <div x-show="isEditing" style="display: none;" class="space-y-4 mt-2 max-w-2xl">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            <label
                                                class="block text-xs font-medium text-blue-700 mb-1">Kecamatan</label>
                                            <select x-model="selectedKecamatan" name="kecamatan_id"
                                        ...
                                        <div>
                                            ...
                                            <select x-model="selectedDesa" name="desa_id"
                                                class="w-full rounded-md border-blue-200 focus:border-blue-500 focus:ring-blue-500 sm:text-sm bg-white disabled:bg-gray-100 disabled:text-gray-400"
                                                :disabled="!selectedKecamatan">
                                                <option value=""
                                                    x-text="selectedKecamatan ? 'Pilih Desa' : 'Pilih Kecamatan Terlebih Dahulu'">
                                                </option>
                                                <template x-for="desa in filteredDesas" :key="desa.id_desa">
                                                    <option :value="String(desa.id_desa)" x-text="desa.nama_desa">
                                                    </option>
                                                </template>
                                            </select>
                                        </div>
                ...
                <div class="bg-white rounded-3xl shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden">
                    ...
                    <div class="p-8">
                        <form method="post" action="{{ route('password.update') }}"
                            class="grid grid-cols-1 md:grid-cols-3 gap-6 items-end" onsubmit="showLoading()">
                            @csrf @method('put')
                            ...
                            <div class="md:col-span-3 flex items-center justify-end gap-4 mt-2">
                                <button type="submit"
                                    class="inline-flex items-center px-6 py-2.5 bg-green-600 border border-transparent rounded-lg font-semibold text-lg text-white hover:bg-green-700 transition shadow-lg shadow-gray-300/50">Simpan
                                    Password</button>
                            </div>
                        </form>
                    </div>
        ...
        <div x-show="showPhotoModal" x-transition:enter="transition ease-out duration-300"
            ...
            <div class="bg-white rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden flex flex-col"
                ...
                <div class="p-4 flex justify-between items-center border-b">
                    ...
                    <button @click="showPhotoModal = false" class="text-gray-400 hover:text-gray-600"><svg
                ...
                <div class="p-6 flex justify-center items-center bg-gray-100">
                    ...
                    @else
                        <div
                            class="w-64 h-64 bg-green-600 flex items-center justify-center text-white text-8xl font-bold select-none rounded-full shadow-lg">
                            {{ strtoupper(substr(Auth::user()->nama_lengkap, 0, 1)) }}</div>
                    @endif
                </div>

                <div class="p-4 border-t flex justify-end space-x-3">
                    <button @click="document.getElementById('profile_photo').click(); showPhotoModal = false;"
                        class="inline-flex justify-center px-4 py-2 bg-blue-600 border border-transparent rounded-lg font-semibold text-sm text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition">
                    ...
                    @if (Auth::user()->profile_photo_path)
                        <form method="POST" action="{{ route('current-user-photo.destroy') }}"
                            onsubmit="showLoading(); showPhotoModal = false;">
                            @csrf @method('DELETE')
                            <button type="submit"
                                class="inline-flex justify-center px-4 py-2 bg-red-600 border border-transparent rounded-lg font-semibold text-sm text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
