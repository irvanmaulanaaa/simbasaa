            ...
            <div class="grid lg:grid-cols-12 gap-12">
                <div class="lg:col-span-8">
                    @php
                        ...
                        if ($media) {
                            ...
                            if (
                                $media->tipe == 'youtube' ||
                                strpos($path, 'youtube.com') !== false ||
                                strpos($path, 'youtu.be') !== false
                            ) {
                                preg_match(
                                ...
                                if ($videoId) {
                                    $mediaHtml =
                                        '<div id="video-container" class="relative w-full aspect-video rounded-[2rem] overflow-hidden shadow-xl mb-8 group ring-1 ring-slate-900/5">
                                                    <iframe id="youtube-iframe" src="https://www.youtube.com/embed/' .
                                        $videoId .
                                        '" 
                                ...
                                $mediaHtml =
                                    '<div class="relative w-full rounded-[2rem] overflow-hidden shadow-xl mb-8 ring-1 ring-slate-900/5 zoom-cursor group" onclick="openImageZoom(\'' .
                                    $imgUrl .
                                    '\')">
                                                <img src="' .
                                    $imgUrl .
                                    '" class="w-full h-auto object-contain bg-white transition-transform duration-300 group-hover:scale-105" alt="' .
                                    $konten->judul .
                                    '">
                    ...
                    <header class="mb-10">
                        <div class="flex items-center justify-between mb-6">
                            <div class="flex items-center space-x-3">
                                ...
                                <span
                                    class="bg-green-100 text-green-700 px-3 py-1 rounded-full font-bold text-xs uppercase tracking-wider">
                            ...
                            <button onclick="toggleLike({{ $konten->id_konten }})"
                                class="flex items-center space-x-2 group px-4 py-1.5 rounded-full bg-white border border-slate-200 hover:border-red-200 hover:bg-red-50 transition-all duration-300 shadow-sm">
                                <svg id="heart-icon"
                                    class="w-5 h-5 transition-all duration-300 {{ $isLiked ? 'text-red-500 fill-red-500' : 'text-slate-400 group-hover:text-red-500' }}"
                                ...
                                <span id="like-count"
                                    class="font-bold text-slate-700 text-sm group-hover:text-red-600 ml-1">{{ $konten->jumlah_like }}</span>
                            </button>
                        </div>
                        <h1 class="text-3xl font-bold text-slate-900 leading-[1.15] tracking-tight mb-4">
                            {{ $konten->judul }}</h1>
                    </header>
                    ...
                    <div class="bg-white rounded-[2rem] p-8 md:p-10 shadow-sm border border-gray-100">
                        ...
                        @auth
                            <form action="{{ route('public.konten.comment', $konten->id_konten) }}" method="POST"
                                class="mb-12 flex gap-4">
                                @csrf
                                <div class="flex-shrink-0 hidden md:block">
                                    <div
                                        class="w-12 h-12 rounded-full bg-green-600 flex items-center justify-center text-white font-bold text-lg shadow-md">
                                        {{ substr(Auth::user()->nama_lengkap, 0, 1) }}
                                    </div>
                                </div>
                                <div class="flex-grow">
                                    <textarea name="isi_komentar" rows="3"
                                        class="w-full border-gray-200 bg-slate-50 rounded-2xl focus:ring-2 focus:ring-green-500 focus:border-green-500 focus:bg-white p-4 transition text-slate-700 placeholder:text-slate-400 resize-none shadow-inner"
                                        placeholder="Bagikan pendapat Anda..." required></textarea>
                                    <div class="mt-3 flex justify-end">
                                        <button type="submit"
                                            class="bg-green-600 hover:bg-green-700 text-white px-6 py-2.5 rounded-xl font-bold shadow-lg shadow-green-200 transition transform hover:-translate-y-0.5">Kirim</button>
                                    </div>
                                </div>
                            </form>
                        @else
                            <div
                                ...
                                <p class="text-slate-600 font-medium mb-4 text-lg">Ingin berkomentar?</p>
                                <a href="{{ route('login') }}"
                                    class="inline-flex items-center px-8 py-3 border border-transparent text-sm font-bold rounded-full text-green-700 bg-green-100 hover:bg-green-200 transition shadow-sm">Login
                                    untuk Berkomentar</a>
                            </div>
                        @endauth
                                            ...
                                            @if (Auth::id() == $komen->user_id)
                                                <form
                                                    ...
                                                    <textarea name="isi_komentar" rows="2"
                                                        class="w-full text-sm border-slate-300 rounded-xl focus:ring-green-500 focus:border-green-500 mb-2 p-3 bg-white">{{ $komen->isi_komentar }}</textarea>
                                                ...
                                                <div class="flex justify-end mt-2 space-x-3 opacity-0 group-hover:opacity-100 transition duration-200"
                                                    ...
                                                    <button onclick="showEditForm({{ $komen->id_komentar }})"
                                                        class="flex items-center text-xs text-slate-400 hover:text-blue-600 transition font-bold">Edit</button>

                                                    <form id="delete-form-{{ $komen->id_komentar }}"
                                                        action="{{ route('public.komentar.delete', $komen->id_komentar) }}"
                                                        method="POST" class="inline">
                                                        @csrf @method('DELETE')
                                                        <button type="button"
                                                            onclick="confirmDeleteComment({{ $komen->id_komentar }})"
                                                            class="flex items-center text-xs text-slate-400 hover:text-red-600 transition font-bold">
                                                            Hapus
                                                        </button>
                                                    </form>
                                                </div>
                ...
                <div class="lg:col-span-4">
                    <div class="sticky top-24 space-y-8">
                        <div
                            class="bg-gradient-to-br from-green-600 to-emerald-700 rounded-[2rem] p-8 text-white shadow-xl relative overflow-hidden group">
                            <div
                                class="absolute top-0 right-0 w-40 h-40 bg-white opacity-5 rounded-full blur-3xl group-hover:scale-150 transition duration-1000">
                            </div>
                            <div
                                class="absolute bottom-0 left-0 w-32 h-32 bg-yellow-300 opacity-10 rounded-full blur-2xl">
                            </div>
                            <div class="relative z-10">
                                <h3 class="text-2xl font-black mb-3 tracking-tight">Dukung Gerakan Hijau!</h3>
                                <p class="text-green-50 text-sm mb-8 leading-relaxed font-medium">Satu langkah kecil
                                    untuk lingkungan, lompatan besar untuk masa depan. Mari berkontribusi bersama kami!
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="bg-white rounded-[2rem] shadow-sm border border-gray-100 p-6"
                            ...
                            <div class="flex border-b border-gray-100 mb-6">
                                <button @click="tab = 'terbaru'"
                                    ...
                                    class="flex-1 pb-3 text-sm font-bold transition">Terbaru</button>
                                <button @click="tab = 'populer'"
                                    ...
                                    class="flex-1 pb-3 text-sm font-bold transition">Populer</button>
                            </div>
                            ...
                            <div x-show="tab === 'terbaru'" class="space-y-6">
                                @foreach ($beritaLain as $item)
                                    <a href="{{ route('public.konten.show', $item->id_konten) }}"
                                        ...
                                        <div
                                            class="w-24 h-20 rounded-2xl overflow-hidden flex-shrink-0 bg-gray-100 relative shadow-sm">
                                            @php
                                                $sMedia = $item->media->first();
                                                $sPath = null;
                                                if ($sMedia) {
                                                    if (
                                                        strpos($sMedia->gambar, 'youtube') !== false ||
                                                        strpos($sMedia->gambar, 'youtu.be') !== false
                                                    ) {
                                                        preg_match(
                                                            "/^(?:http(?:s)?:\/\/)?(?:www\.)?(?:m\.)?(?:youtu\.be\/|youtube\.com\/(?:(?:watch)?\?(?:.*&)?v(?:i)?=|(?:embed|v|vi|user|shorts)\/))([^\?&\"'>]+)/",
                                                            $sMedia->gambar,
                                                        ...
                                                        $sPath = filter_var($sMedia->gambar, FILTER_VALIDATE_URL)
                                                            ? $sMedia->gambar
                                        ...
                                        <div class="flex-grow">
                                            <h4
                                                class="text-sm font-bold text-slate-800 group-hover:text-green-600 leading-snug line-clamp-2 transition mb-1">
                                                {{ $item->judul }}</h4>
                                            <span
                                                class="text-[10px] font-bold text-slate-400 uppercase tracking-wide">{{ $item->created_at->format('d M Y') }}</span>
                                        </div>
                                    </a>
                                @endforeach
                            </div>
                            ...
                            <div x-show="tab === 'populer'" class="space-y-6" style="display: none;">
                                ...
                                @foreach ($populer as $idx => $item)
                                    <a href="{{ route('public.konten.show', $item->id_konten) }}"
                                        ...
                                        <span
                                            class="text-3xl font-black text-slate-200 group-hover:text-green-200 transition w-8 text-center italic">{{ $idx + 1 }}</span>
                                        <div>
                                            <h4
                                                class="text-sm font-bold text-slate-800 group-hover:text-green-600 leading-snug line-clamp-2 transition">
                                                {{ $item->judul }}</h4>
                                            <div
                                                class="flex items-center gap-1 mt-1 text-xs text-slate-400 font-medium">
                                                <svg class="w-3 h-3 text-red-400" fill="currentColor"
                                                    viewBox="0 0 20 20">
                                                    <path
